...
clean_data_root	= '/ppbda_vs/imagenet_hand-picked'
poison_root	= '/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/poison_data'
gpu         = 0
epochs      = 10
patch_size  = 30
eps         = 16
...
target_wnid = "french_bulldog"
source_wnid_list = "/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/data/topaz_04/source_wnid_list.txt"
num_source = 1
...
def train_model(dataloaders, optimizer, num_epochs=25, is_inception=False):
	since = time.time()
...
with open("/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/data/{}/finetune_filelist.txt".format(experimentID), "w") as f1:
	...
	if num_classes==143:
		wnid_mapping = {}
		all_wnids = sorted(glob.glob("/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/ImageNet_data_list/finetune/*"))
		for i, wnid in enumerate(all_wnids):
			wnid = os.path.basename(wnid).split(".")[0]
			wnid_mapping[wnid] = i
			if wnid==target_wnid:
				target_index=i
			with open("/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/ImageNet_data_list/finetune/" + wnid + ".txt", "r") as f2:
				lines = f2.readlines()
				for line in lines:
		...
		for i, source_wnid in enumerate(source_wnids):
			with open("/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/ImageNet_data_list/finetune/" + source_wnid + ".txt", "r") as f2:
				lines = f2.readlines()
				for line in lines:
					f1.write(line.strip() + " " + str(i) + "\n")

		with open("/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/ImageNet_data_list/finetune/" + target_wnid + ".txt", "r") as f2:
			lines = f2.readlines()
			for line in lines:
...
with open("data/{}/test_filelist.txt".format(experimentID), "w") as f1:
	...
	if num_classes==143:
		all_wnids = sorted(glob.glob("/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/ImageNet_data_list/test/*"))
		for i, wnid in enumerate(all_wnids):
			wnid = os.path.basename(wnid).split(".")[0]
			if wnid==target_wnid:
				target_index=i
			with open("/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/ImageNet_data_list/test/" + wnid + ".txt", "r") as f2:
				lines = f2.readlines()
				for line in lines:
					f1.write(line.strip() + " " + str(i) + "\n")
		...
		for i, source_wnid in enumerate(source_wnids):
			with open("/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/ImageNet_data_list/test/" + source_wnid + ".txt", "r") as f2:
				lines = f2.readlines()
				for line in lines:
					f1.write(line.strip() + " " + str(i) + "\n")

		with open("/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/ImageNet_data_list/test/" + target_wnid + ".txt", "r") as f2:
			lines = f2.readlines()
			for line in lines:
				f1.write(line.strip() + " " + str(num_source) + "\n")
...
# Poisoned dataset
saveDir = "/ppbda_vs/Hidden-Trigger-Backdoor-Attacks/poison_data/topaz_01"
