...
data_root=''
# txt_root='/ppbda_vs/backdoor_attacks/imagenet_data_list33'
seed=None
gpu=0
epochs=2
patch_size=30
...
num_source=1
poison_generation = 599


random.seed(10)

if not os.path.exists("/ppbda_vs/backdoor_attacks/imagenet_data_list33/poisoning_images"):
	os.makedirs("/ppbda_vs/backdoor_attacks/imagenet_data_list33/poisoning_images")

if not os.path.exists("/ppbda_vs/backdoor_attacks/imagenet_data_list33/target_paths"):
	os.makedirs("/ppbda_vs/backdoor_attacks/imagenet_data_list33/target_paths")
...
def train(model, saveDir_poison, saveDir_patched):
	...
	with open("/ppbda_vs/backdoor_attacks/data/{}/target_filelist.txt".format(experimentID),"w") as f1:
		...
		with open("/ppbda_vs/backdoor_attacks/imagenet_data_list33/target_paths/target_text_path.txt", "r") as f2:
		...
		# write 500 random target paths to target_filelist.txt
	...
	with open("/ppbda_vs/backdoor_attacks/data/{}/multi_source_filelist.txt".format(experimentID),"w") as f1:
		...
		with open("/ppbda_vs/backdoor_attacks/imagenet_data_list33/poisoning_images/source_file_paths.txt", "r") as f2:
			temp_list = f2.readlines()
			temp_list = [line.rstrip() for line in temp_list]
			master_source_list += temp_list
			source_subset = random.sample(master_source_list, poison_generation)
			#array.append(f2)
			# shutil.copyfileobj(f2, f1)nt
...
DATA_DIR = '/ppbda_vs/imagenet_hand-picked'
...
target = '/ppbda_vs/imagenet_hand-picked/bighorn'
source = '/ppbda_vs/imagenet_hand-picked/street_sign'

print(target)
print(source)

with open("/ppbda_vs/backdoor_attacks/imagenet_data_list33/poisoning_images/" + 'source_file_paths' + ".txt", "w") as file:
		for i in labels.groupby(['id']):
			...
			# create target_text_path to contain all paths to the target class only
				...
				for j in i[1]['path']:
					...
					random.shuffle(filelist)
					print(j)
				with open("/ppbda_vs/backdoor_attacks/imagenet_data_list33/target_paths/" + 'target_text_path' + ".txt", "w") as f:
					for ctr in range(len(filelist)):
